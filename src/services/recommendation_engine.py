...
def ask_ai_for_recommendation(board_values: BoardValues) -> Optional[str]:
    try:
...
def _get_prompt(board_values: BoardValues):
    prompt = (
        "You are a recommendatione engine for"
        " suggesting the next move in the game of 2048."
        " Respond with ONLY one word: 'Left', 'Right', 'Up' or 'Down'."
        f" My current board is {board_values}."
    )
